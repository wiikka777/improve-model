...
class Wrap_Dataset4(Dataset):
    ...
    def __init__(self, X, y, weight, y1, y2, comment_weights, use_cuda=True):
        if comment_weights is None:
            comment_weights = np.zeros((len(X), 6), dtype=np.float32)
        if use_cuda:
            self.X = torch.LongTensor(X).cuda()
            self.y = torch.Tensor(y).cuda()
            self.weight = torch.Tensor(weight).cuda()
            ...
            self.comment_weights = torch.Tensor(comment_weights).cuda()
        else:
            self.X = torch.LongTensor(X).cpu()
            self.y = torch.Tensor(y).cpu()
            self.weight = torch.Tensor(weight).cpu()
            ...
            self.comment_weights = torch.Tensor(comment_weights).cpu()
