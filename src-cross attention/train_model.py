...
class Learner(object):
    ...
    def __init__(self, args):
        ...
        # 加载视频嵌入
        print("Loading pre-computed video embeddings...")
        self.photo_embeddings = torch.load('/user/zhuohang.yu/u24922/.project/dir.project/rec_datasets/KuaiComt/bert-embeddings.pt').to(dtype=torch.float32).cuda()
        self.photo_embeddings.requires_grad = False  # 冻结嵌入

        # 加载评论嵌入 
        print("Loading pre-computed comment embeddings...")
        self.comment_embeddings = torch.load('/user/zhuohang.yu/u24922/.project/dir.project/rec_datasets/KuaiComt/bert-embeddings_comments.pt').to(dtype=torch.float32).cuda()
    ...
    def _load_and_spilt_dat(self):
        if self.dat_name == 'KuaiComt':
            all_dat = pd.read_csv('/user/zhuohang.yu/u24922/.project/dir.project/rec_datasets/WM_KuaiComt/KuaiComt_subset.csv')
    ...
    def _wrap_dat(self):
        ...
        input_vali = Wrap_Dataset4(make_feature_with_comments(self.vali_dat, self.dat_name),
                                self.vali_dat[self.label_name].tolist(),
                                self.vali_dat[self.weight_name].tolist(),
                                self.vali_dat[self.label1_name].tolist(),
                                self.vali_dat[self.label2_name].tolist(),
                                None,
                                False)
        ...
        input_test = Wrap_Dataset4(make_feature_with_comments(self.test_dat, self.dat_name),
                                self.test_dat[self.label_name].tolist(),
                                self.test_dat[self.weight_name].tolist(),
                                self.test_dat[self.label1_name].tolist(),
                                self.test_dat[self.label2_name].tolist(),
                                None,
                                False)
    ...
    def _train_iteration(self):
        ...
        for epoch in range(self.epoch_num):
            ...
            for _id, batch in enumerate(self.train_loader):
                ...
                # batch: [features, label, weight, y1_list, y2_list, comment_weights]
                self.c_model.train()
