...
def pre_kuaicomt():
    ...
    # KuaiComt - 使用绝对路径
    base_data_path = '/user/zhuohang.yu/u24922/.project/dir.project/rec_datasets/KuaiComt'
    
    df_kuaiComt_interaction_ls = []
    file_names = [f'{base_data_path}/user_photo_final_filtered_table.csv']
    for file_name in file_names:
        df_kuaiComt_interaction_ls.append(pd.read_csv(file_name, sep='\t'))
    df_kuaiComt_usr_fe = pd.read_csv(f'{base_data_path}/user_table_final.csv',sep='\t')
    df_kuaiComt_video_fe_basic = pd.read_csv(f'{base_data_path}/photo_table_final.csv',sep='\t', lineterminator='\n')
