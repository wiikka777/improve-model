...
class My_DeepCrossNetworkModel_withCommentsRanking(nn.Module):
    def __init__(self, field_dims, comments_dims, embed_dim, num_layers, mlp_dims, dropout, text_embeddings, 
        ...
        # 恢复原结构：拼接embedding和权重后用Linear映射
    ...
    def forward(self, x, explicit_weights=None):
        ...
        if explicit_weights is not None:
            ...
            # DEBUG
            print(f"[DEBUG-dcn.py] explicit_weights shape: {w.shape}, sample values: {w[0]}")
        ...
        # video_id is at index 6 (8 base fields - user, follow_range, register_range, fans_range, friend_range, active_degree, video_id, author_id + 2 user interaction fields is_like, is_follow)
        text_embed_ids = x[:, 6]
