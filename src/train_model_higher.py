...
class Learner(object):
    ...
    def __init__(self, args):
        ...
        self.photo_embeddings = torch.load('/user/zhuohang.yu/u24922/csp/rec_datasets/KuaiComt/bert-embeddings.pt').to(dtype=torch.float32).cuda()
        print("Loading pre-computed text embeddings...")
        self.photo_embeddings.requires_grad = False  # 冻结嵌入

        # 加载评论的BERT嵌入
        self.comment_embeddings = torch.load('/user/zhuohang.yu/u24922/csp/rec_datasets/KuaiComt/bert-embeddings_comments.pt').to(dtype=torch.float32).cuda()
