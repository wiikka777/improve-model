...
def _get_pred(data_ld, model):
    ...
    with torch.no_grad():
        ...
        for _id, batch in enumerate(data_ld):
            ...
            # 如果 batch 中携带显式权重（位于第6个位置），则传递给模型
            explicit_weights = None
            # train dataloader returns 6-tuple, vali/test returns 4-tuple (X,y,weight,comment_weights)
            if len(batch) >= 6:
                try:
                    explicit_weights = batch[5].to(model_device)
                except Exception:
                    explicit_weights = None
            elif len(batch) == 4:
                # treat 4th element as comment_weights when present
                try:
                    explicit_weights = batch[3].to(model_device)
                except Exception:
                    explicit_weights = None
            if explicit_weights is not None:
                pred_batch = model(x, explicit_weights=explicit_weights).view(x.size(0))
            else:
            ...
            for _id, batch in enumerate(data_ld):
                ...
                if len(batch) >= 6:
                    ...
                    try:
                        ...
                        explicit_weights = None
                if explicit_weights is not None:
                    pred_batch_m, pred_batch_c = model(x, explicit_weights=explicit_weights)
                else:
