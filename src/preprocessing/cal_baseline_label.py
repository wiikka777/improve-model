...
def cal_pcr_label(df_dat):
    ...
    df_dat['PCR_st'] =  np.nan_to_num(df_dat['comment_stay_time'].values/df_dat['duration_ms'].values, nan=0.0, posinf=0.0, neginf=0.0)
    assert np.all(np.isfinite(df_dat['PCR_st'].values))
    print(np.all(np.isfinite(df_dat['PCR_st'].values)))
    return df_dat
...
# def cal_quantile_label(df_dat, group_num):
    ...
    df_dat['st_rank'] = df_dat.groupby('quantile_bin')['comment_stay_time'].rank(method='dense', ascending=True)
    group_sample_num = df_dat.groupby('quantile_bin')['st_rank'].max().values
    df_dat['percentile_st'] = (df_dat['st_rank'])/group_sample_num[df_dat['quantile_bin']]

    df_dat.drop(['wt_rank'], axis=1, inplace=True)
    # df_dat.drop(['mean_play'], axis=1, inplace=True)
    # df_dat.drop(['std_play'], axis=1, inplace=True)
    df_dat.drop(['st_rank'], axis=1, inplace=True)
    return df_dat
...
def cal_quantile_label(df_dat, group_num):
    ...
    # 删除临时列
    df_dat.drop(['wt_rank', 'st_rank'], axis=1, inplace=True)
...
def cal_wlr_label(df_dat):
    ...
    df_dat['weighted_st'] = df_dat['comment_stay_time'].apply(lambda x: x if x > 0 else 1)
    return df_dat
...
def cal_ndt_label(df_dat):
    ...
    df_dat['log_st'] = np.log(df_dat['comment_stay_time'].values + 1)
    st_vr_bound = np.exp(df_dat['log_st'].mean() - df_dat['log_st'].std())
    print('st_vr_bound:', st_vr_bound)
    df_dat['valid_stay'] = df_dat['comment_stay_time'].apply(lambda x: 1 if x > st_vr_bound else 0)
    # kuairand_dat['NDT'] = kuairand_dat['play_time_truncate'].apply(lambda x: (A/(1+np.exp(-(x-offset)/tau)))-B)
    df_dat['NDT_st'] = df_dat['comment_stay_time'].apply(lambda x: (2.319/(1+np.exp(-(x-st_vr_bound)/20)))-0.744)
    return df_dat
